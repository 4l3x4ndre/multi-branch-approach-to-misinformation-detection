    ...
    for batch in tqdm(loader):
        ...
        for idx, (graph, kg) in enumerate(zip(graphs, aligned_kg_graphs)):
            ...
            if graph is torch_nan or kg is torch_nan or len(kg.nodes) == 0 or len(graph.nodes) == 0:
                ...
                nt = Network(height='1000px', width='1000px', directed=True)
                nt.from_nx(graph)
                for node in nt.nodes:
                    node['color'] = "#95BB63"
                nt.show(f'reports/graph{step}_claim.html', notebook=False)
                nt.from_nx(kg)
                for node in nt.nodes:
                    node['color'] = "#618AAE"
                nt.show(f'reports/graph{step}_kg.html', notebook=False)
